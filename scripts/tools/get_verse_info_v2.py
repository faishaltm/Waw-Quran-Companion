#!/usr/bin/env python3
"""
Get complete linguistic information for a Quranic verse or verse range (Version 2)
Uses natural language text output for balaghah features - LLM-friendly format
...
def extract_verse_info_compact(chapter_num, verse_start, verse_end, comprehensive_data):
    ...
    for verse_data in verses_data:
        ...
        # Get root repetitions for this verse (needed for text conversion)
        root_reps_for_text = contextual_roots.get(verse_num) if verse_num in contextual_roots else None

        # Convert balaghah and words to natural language text (V2 feature)
        # Now includes section heading and root repetitions in the narrative
        text_analysis = convert_verse_to_text(
            verse_output,
            root_meanings,
            section_heading=section_heading_for_text,
            root_repetitions=root_reps_for_text
        )

        # Create final text-based output
        final_output = {
            'verse_number': verse_num,
            'text': verse_data['text'],
        ...
        # Add tafsir and asbab if available (simplified format)
        tafsir = verse_data.get('tafsir')
        if tafsir:
            simplified_tafsir = simplify_tafsir(tafsir)
            if simplified_tafsir:
        ...
        # Add text analysis and key words
        if text_analysis.get('analysis'):
            final_output['analysis'] = text_analysis['analysis']
        if text_analysis.get('key_words'):
            final_output['key_words'] = text_analysis['key_words']

        # Add contextual root repetitions with section heading context (structured data)
        if root_reps_for_text:
            final_output['root_repetitions'] = root_reps_for_text
...
def main():
    ...
    print("=" * 60)
    print("Quranic Verse Information Retrieval (Version 2)")
    print("Natural Language Text Format - LLM-Friendly")
    print("=" * 60)
    print()
    ...
    print()
    print("Output: Text analysis of balaghah features with key word meanings")
    print()
