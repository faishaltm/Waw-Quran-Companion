#!/usr/bin/env python3
"""
Get complete linguistic information for a Quranic verse or verse range (Version 3)
Uses PURE JSON output for balaghah features - structured data format
...
# Add path to loaders and converters directories
...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'balaghah_detectors'))
...
def simplify_tafsir(tafsir_data):
    """
    Simplify tafsir structure to simple source: content format.

    Input: {
        'kashshaf_arabic': {'text': '...', 'verses_range': [1,2,3], 'source': '...', 'methodology': '...'},
        'maarif_en': {'text': '...', 'source': '...', 'methodology': '...'},
        'ibn_kathir_en': {'text': '...', 'source': '...', 'methodology': '...'}
    }

    Output: {
        'Al-Kashshaf (Arabic)': 'content... [Covers verses 1-3]',
        'Ma\'arif al-Qur\'an (English)': 'content...',
        'Tafsir Ibn Kathir (English)': 'content...'
    }
    """
    if not tafsir_data:
        return None
    ...
    # Al-Kashshaf (Arabic - with verses_range info)
    if 'kashshaf_arabic' in tafsir_data:
        k = tafsir_data['kashshaf_arabic']
        content = k.get('text', '')
        verses_range = k.get('verses_range', [])

        # Add verses range info if multi-verse commentary
        if verses_range and len(verses_range) > 1:
            verse_start = verses_range[0]
            verse_end = verses_range[-1]
            content = f"{content}\n\n[Note: This commentary covers verses {verse_start}-{verse_end}]"

        simplified['Al-Kashshaf (Arabic)'] = content

    # Ma'arif al-Qur'an (English)
    if 'maarif_en' in tafsir_data:
        m = tafsir_data['maarif_en']
        simplified['Ma\'arif al-Qur\'an (English)'] = m.get('text', '')

    # Tafsir Ibn Kathir (English)
    if 'ibn_kathir_en' in tafsir_data:
        i = tafsir_data['ibn_kathir_en']
        simplified['Tafsir Ibn Kathir (English)'] = i.get('text', '')

    return simplified if simplified else None


def simplify_asbab_nuzul(asbab_list):
    """
    Simplify asbab nuzul to concise format, merging duplicates.

    Input: [
        {'chapter': 70, 'verse': 1, 'occasion': '...', 'full_narrative': '...', 'verse_range': '1'},
        {'chapter': 70, 'verse': 1, 'occasion': '...', 'full_narrative': '...', 'verse_range': '1'}
    ]

    Output: [
        'Occasion 1: narrative text',
        'Occasion 2: narrative text'
    ]

    Removes redundant fields (chapter, verse, surah_name already known from context).
    Merges occasion and full_narrative (often same or similar).
    Deduplicates identical narratives.
    """
    if not asbab_list:
        return None

    # Collect unique narratives
    narratives = []
    seen = set()

    for entry in asbab_list:
        # Use full_narrative if available and different from occasion
        occasion = entry.get('occasion', '').strip()
        full_narrative = entry.get('full_narrative', '').strip()

        # Choose the more complete text
        if full_narrative and full_narrative != occasion:
            text = full_narrative
        elif occasion:
            text = occasion
        else:
    ...
    # If only one narrative, return it directly
    if len(narratives) == 1:
        return narratives[0]

    # If multiple, number them
    return narratives
...
def distribute_balaghah_to_verses(verses_data, lemmas_dict, chapter_num, comprehensive_data):
    ...
    # Store root data for later contextual use (but don't add to chapter_balaghah)
    # This will be handled by add_contextual_root_repetitions() instead
    ...
    for verse_data in verses_data:
        ...
        # NOTE: Root repetitions are now handled separately by add_contextual_root_repetitions()
        # with section heading context - not added here anymore
        ...
        # NOTE: Tibaq and Tashbih detection moved to main loop after morphology is loaded
        # (see lines ~1920+ where morphology_by_word is available)
    ...
    # Return root data for contextual repetitions
    root_data = {
        'repeated_roots': repeated_roots,
        'root_to_lemmas': root_to_lemmas,
        'root_to_lemmas_buckwalter': root_to_lemmas_buckwalter
    }

    return chapter_balaghah, verse_balaghah, root_data
...
def add_contextual_root_repetitions(verses_data, repeated_roots, root_to_lemmas, root_to_lemmas_buckwalter,
                                     lemmas_dict, chapter_num, metadata_loader, verse_start, verse_end,
                                     comprehensive_data):
    """Add contextual root repetition information to verses.

    For the first occurrence of a root in the extraction range, adds full context including
    section headings and verse text for all verses where the root appears.

    Args:
        verses_data: List of verse dictionaries
        repeated_roots: Dict of {root: set of verses} for roots appearing 2+ times
        root_to_lemmas: Dict of {root: set of lemmas in Arabic}
        root_to_lemmas_buckwalter: Dict of {root: set of lemmas in Buckwalter}
        lemmas_dict: Lemma dictionary for translations
        chapter_num: Current chapter number
        metadata_loader: MetadataLoader for section info
        verse_start: Start of extraction range
        verse_end: End of extraction range
        comprehensive_data: Full data to get verse text

    Returns:
        Dict mapping verse_number -> root_repetitions data
    """
    verse_root_reps = {}
    shown_roots = set()  # Track roots already shown in this extraction

    # Get chapter data for verse text lookup
    chapter_data = None
    ...
    for verse_data in verses_data:
        ...
        # Find which roots appear in this verse
        words = verse_data.get('words', [])
        for word in words:
            morphology = word.get('morphology', [])
            if morphology:
                ...
                if root_buck:
                    ...
                    # FILTER: Skip common auxiliary roots (too frequent, no thematic value)
                    if root_arabic in ['كون', 'شيء', 'كلل']:
                        continue

                    # Check if this root is repeated in the chapter
                    if root_arabic in repeated_roots:
                        all_verses = repeated_roots[root_arabic]

                        # Check if this is the first time we're showing this root in extraction
                        if root_arabic not in shown_roots:
                            # FIRST OCCURRENCE in extraction - show full context
                            shown_roots.add(root_arabic)

                            # Get context for all verses with this root
                            verse_contexts = []
                            for v in sorted(all_verses):
                                if v == verse_num:
                                    continue  # Skip current verse

                                context = {'verse': v}

                                # Get verse text (Arabic and English)
                                if chapter_data:
                                    for cv in chapter_data['verses']:
                                        if cv['verse_number'] == v:
                                            context['text'] = cv['text']
                                            context['translation'] = cv.get('translation_en', '')
                                            break

                                # Add section heading if available
                                if metadata_loader:
                                    section = metadata_loader.get_section_for_verse(chapter_num, v)
                                    if section:
                                        context['section_heading'] = section['heading']

                                verse_contexts.append(context)

                            # Build full root entry
                            root_entry = {
                                'first_occurrence_in_extraction': True,
                                'lemmas': sorted(list(root_to_lemmas[root_arabic])),
                                'total_occurrences_in_chapter': len(all_verses),
                                'other_verses': verse_contexts
                            }

                            # Add translations if available
                            if lemmas_dict:
                                translations = []
                                for buck in root_to_lemmas_buckwalter[root_arabic]:
                                    trans = get_lemma_translation(buck, lemmas_dict)
                                    if trans:
                                        translations.append(trans)
                                if translations:
                            ...
                            verse_roots[root_arabic] = root_entry

                        else:
                            # SUBSEQUENT OCCURRENCE - just note it
                            # Find the first verse in extraction where this root appeared
                            first_verse_in_extraction = None
                            for v in sorted(all_verses):
                                if verse_start <= v <= verse_end and v < verse_num:
                                    first_verse_in_extraction = v
                                    break

                            verse_roots[root_arabic] = {
                                'first_occurrence_in_extraction': False,
                                'note': f"Root already mentioned at verse {first_verse_in_extraction}" if first_verse_in_extraction else "Root mentioned earlier in chapter"
                            }

        if verse_roots:
            verse_root_reps[verse_num] = verse_roots

    return verse_root_reps


def extract_verse_info_compact(chapter_num, verse_start, verse_end, comprehensive_data, metadata_loader=None):
    """
    ...
    Args:
        ...
        metadata_loader: Optional MetadataLoader instance (reuse to avoid repeated loading)
    """
    ...
    # Use provided metadata_loader or create new one
    ...
    has_morphology = metadata_loader is not None and metadata_loader.morphology_segments is not None
    ...
    # Add comprehensive surah info if available (from metadata_loader)
    if metadata_loader:
        # Get Quran.com comprehensive context (Maududi's Tafhim)
        qurancom_context = metadata_loader.get_qurancom_context(chapter_num)
        if qurancom_context:
            # Use full text introduction from Quran.com API
            result['metadata']['introduction'] = qurancom_context.get('text', '')
            result['metadata']['introduction_source'] = qurancom_context.get('source', 'Sayyid Abul Ala Maududi - Tafhim al-Quran')
        else:
            # Fallback to Ibn Kathir if Quran.com not available
            surah_info = metadata_loader.get_surah_info(chapter_num)
            if surah_info and 'introduction' in surah_info:
                result['metadata']['introduction'] = surah_info['introduction']
                result['metadata']['introduction_source'] = 'Ibn Kathir'

        # Add name_translation
        surah_info = metadata_loader.get_surah_info(chapter_num)
        if surah_info and 'name_translation' in surah_info:
            result['metadata']['name_translation'] = surah_info['name_translation']

        # Add revelation order and mixed revelation info from Tanzil
        revelation_order_data = metadata_loader.get_revelation_order(chapter_num)
        if revelation_order_data:
            result['metadata']['revelation_order'] = revelation_order_data.get('revelation_order')
            mixed_rev = revelation_order_data.get('mixed_revelation')
            if mixed_rev:
                result['metadata']['mixed_revelation'] = mixed_rev

        # Add chapter section headings to metadata for LLM context
        # Format: {"1-7": "Excellence of the Prophet", "8-16": "Warning About Deniers"}
        chapter_sections = metadata_loader.get_chapter_sections(chapter_num)
        if chapter_sections:
            sections_dict = {}
            for section in chapter_sections:
                verse_range = f"{section['verse_start']}-{section['verse_end']}"
                sections_dict[verse_range] = section['heading']
            result['metadata']['section_headings'] = sections_dict
    ...
    chapter_balaghah, verse_balaghah_map, root_data = distribute_balaghah_to_verses(verses_data, lemmas_dict, chapter_num, comprehensive_data)

    # Generate contextual root repetitions with section headings and verse text
    contextual_roots = add_contextual_root_repetitions(
        verses_data,
        root_data['repeated_roots'],
        root_data['root_to_lemmas'],
        root_data['root_to_lemmas_buckwalter'],
        lemmas_dict,
        chapter_num,
        metadata_loader,
        verse_start,
    ...
    for verse_data in verses_data:
        ...
        # Add tafsir if available (simplified format)
        tafsir = verse_data.get('tafsir')
        if tafsir:
            simplified_tafsir = simplify_tafsir(tafsir)
            if simplified_tafsir:
                verse_output['tafsir'] = simplified_tafsir

        # Add asbab nuzul if at start of range (simplified format)
        ...
        if asbab:
            ...
            if filtered_asbab:
                simplified_asbab = simplify_asbab_nuzul(filtered_asbab)
                if simplified_asbab:
                    verse_output['asbab_nuzul'] = simplified_asbab
        ...
        if has_morphology and metadata_loader:
            ...
            if verse_morphology:
                ...
                for word_num_str in sorted(verse_morphology.keys(), key=lambda x: int(x)):
                    ...
                    # Add word number for balaghah detectors
                    word_morphology['word_number'] = word_num
        ...
        # DETECT: Taqdim wa Ta'khir (Word Order) - requires dependencies
        if verse_deps and morphology_by_word:
            taqdim_result = detect_taqdim_takhir(verse_deps, morphology_by_word)
            if taqdim_result:
                # Add to balaghah section of verse_output
                if 'balaghah' not in verse_output:
                    verse_output['balaghah'] = {}
                verse_output['balaghah']['taqdim_takhir'] = taqdim_result

        # DETECT: Tibaq (Antithesis) and Tashbih (Simile) - requires morphology
        if morphology_by_word:
            # Prepare data for tibaq detector
            verse_words_for_tibaq = []
            for word_morph in morphology_by_word:
                root = word_morph.get('root')
                if root:
                    verse_words_for_tibaq.append({
                        'root': root,  # Already in Arabic from simplify_word_morphology
                        'lemma': word_morph.get('lemma'),
                        'text': word_morph.get('text', ''),
                        'word_number': word_morph.get('word_number')
                    })

            # Detect tibaq
            tibaq_result = detect_tibaq(verse_words_for_tibaq)
            if tibaq_result:
                if 'balaghah' not in verse_output:
                    verse_output['balaghah'] = {}
                verse_output['balaghah']['tibaq'] = tibaq_result

            # Prepare data for tashbih detector
            verse_morphology_for_tashbih = []
            for word_morph in morphology_by_word:
                verse_morphology_for_tashbih.append({
                    'word_number': word_morph.get('word_number'),
                    'text': word_morph.get('text', ''),
                    'root': word_morph.get('root'),
                    'lemma': word_morph.get('lemma')
            ...
            # Detect tashbih - pass raw segments for better particle detection
            tashbih_result = detect_tashbih_candidates(
                verse_morphology_for_tashbih,
                verse_segments=verse_morphology  # Pass raw segments
            )
            if tashbih_result:
                if 'balaghah' not in verse_output:
                    verse_output['balaghah'] = {}
                verse_output['balaghah']['tashbih'] = tashbih_result

        # Get section heading info for context (needed for text conversion)
        section_heading_for_text = None
        if metadata_loader:
            section_info = metadata_loader.get_section_for_verse(chapter_num, verse_num)
            if section_info:
                # Calculate position within section
                section_position = verse_num - section_info['verse_start'] + 1
                section_total = section_info['verse_end'] - section_info['verse_start'] + 1
                section_heading_for_text = {
                    'heading': section_info['heading'],
                    'verse_start': section_info['verse_start'],
                    'verse_end': section_info['verse_end'],
                    'position': f"verse {section_position} of {section_total}"
                }

        # Get root repetitions for this verse
        root_reps_for_verse = contextual_roots.get(verse_num) if verse_num in contextual_roots else None

        # Create final JSON-based output (V3 feature - pure structured data)
        final_output = {
            'verse_number': verse_num,
            'text': verse_data['text'],
        ...
        if tafsir:
            simplified_tafsir = simplify_tafsir(tafsir)
            if simplified_tafsir:
                final_output['tafsir'] = simplified_tafsir

        # Add asbab nuzul if available
        if verse_output.get('asbab_nuzul'):
            final_output['asbab_nuzul'] = verse_output['asbab_nuzul']

        # Add PURE JSON balaghah features (not text)
        if verse_output.get('balaghah'):
            final_output['balaghah'] = verse_output['balaghah']

        # Add contextual root repetitions (pure JSON)
        if root_reps_for_verse:
            final_output['root_repetitions'] = root_reps_for_verse

        # Add words morphology (pure JSON)
        if verse_output.get('words'):
            final_output['words'] = verse_output['words']

        result['verses'].append(final_output)

    # NOTE: chapter_balaghah removed - root repetitions now appear inline with verses
    # with contextual information (section headings, ruku numbers) for related verses
...
def main():
    ...
    print("=" * 60)
    print("Quranic Verse Information Retrieval (Version 3)")
    print("Pure JSON Format - Structured Balaghah Data")
    print("=" * 60)
    print()
    ...
    print()
    print("Output: Pure JSON balaghah features with structured morphology")
    print()
